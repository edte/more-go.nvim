...
Return.enable_autocmds = function()
    ...
    command_id = vim.api.nvim_create_autocmd(
        ...
        { callback = Return.wrap_golang_return }
    )
end
...
Return.wrap_golang_return = function()
...
Return.setup = function(config)
    Return.config = vim.tbl_deep_extend("force", Return.config, config)

    if Return.config.enable_autocmds then
        Return.enable_autocmds()
    end

    Return.setup_user_commands()
end
